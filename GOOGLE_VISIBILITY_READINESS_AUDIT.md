...
| 2 | Write unique meta descriptions for the 8 pages sharing the generic fallback, plus `rotten_fencepost.html` and `aloha_wellness_funnel.html` | High | Medium | Code-only | **DONE.** 8 pages + `rotten_fencepost.html` — Work Order 005 (2nd). `aloha_wellness_funnel.html` — status was left stale here; it already has a unique `<title>` and `<meta name="description">`, added incidentally as part of Work Order 006's Open Graph/Twitter work (confirmed still present, non-generic, in code as of the final roadmap audit conducted after Work Order 010's completion). |
...
| 4 | Add `noindex` to transactional pages (`/checkout/*`, success/thank-you pages) | Medium | Low | Code-only | **DONE — Work Order 007.** (Status was left stale here; the `noindex` work happened alongside the structured-data implementation — see `CLAUDE.md`.) |
...
| 6 | Resolve orphan pages — link `/product/rotten_fencepost_field_guide` from somewhere public, or confirm intentional and leave as-is (`/aloha-wellness` is now in the sitemap as of Work Order 005, which helps discovery but doesn't add an internal link to it) | Medium | Low | Code-only | Partially addressed. **Work Order 008** added 4 natural contextual links elsewhere on the site (see Section 6). **Work Order 010** added one natural inbound link to each of the 4 product pages Work Order 009 added to the sitemap (`prod_aloha_wellness`, `prod_kingdom_booklet1`, `prod_kingdom_booklet2`, `prod_nahenahe_cd`), from their existing related content page. Still deliberately untouched: `/product/rotten_fencepost_field_guide` (WO009 intentionally excluded it from the sitemap as a near-duplicate of `/rotten-fencepost`, so it's not treated as a discovery gap), the `/aloha-wellness` hyphen funnel and `/myron-golden` (WO008 deliberately excluded these — ad-landing pages, not organic content), and `partner_tier1`-`4` (still an open editorial call, not a clear-cut gap, per Section 6). |
